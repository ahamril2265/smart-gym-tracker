...
export default function AdminDashboard() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editUser) {
                ...
                await axios.put(
                    `http://localhost:5001/api/admin/users/${editUser.id}`,
                    formData,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
    ...
    const handleTrainerAssign = async (userId, trainerId) => {
        try {
            await axios.put(
                `http://localhost:5001/api/admin/users/${userId}/trainer`,
                ...
                <div className="flex gap-2">
                    <button
                        onClick={async () => {
                            ...
                            try {
                                const res = await axios.post("http://localhost:5001/api/admin/notify-expiry", {}, {
                                    headers: { Authorization: `Bearer ${token}` }
                                });
